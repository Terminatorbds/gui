...
const { randomEmail, randomName } = require('../utils/random');
...
describe('Auth routes', () => {
  describe('POST /v1/auth/register', () => {
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      ...
      expect(emailService.sendVerificationEmail).toHaveBeenCalledWith(newUser.email, expect.any(String));
      const verifyEmailToken = emailService.sendVerificationEmail.mock.calls[0][1];
      const dbVerifyEmailToken = await Token.findOne({ token: verifyEmailToken, user: dbUser.id, type: tokenTypes.VERIFY_EMAIL });
    ...
    test('should still return 201 if verification email delivery fails', async () => {
      emailService.sendVerificationEmail.mockRejectedValueOnce(
        new Error('all recipients were rejected: demo domains can only send to account owners')
      );
      ...
      const dbVerifyEmailToken = await Token.findOne({ user: dbUser.id, type: tokenTypes.VERIFY_EMAIL });
      expect(dbVerifyEmailToken).toBeDefined();
    });
  ...
  describe('GET /v1/auth/verify-email', () => {
    test('should verify the email, issue auth tokens, and redirect to the frontend profile', async () => {
      await insertUsers([userOne]);
      const expires = moment().add(config.jwt.verifyEmailExpirationMinutes, 'minutes');
      const verifyEmailToken = tokenService.generateToken(userOne._id, expires);
      await tokenService.saveToken(verifyEmailToken, userOne._id, expires, tokenTypes.VERIFY_EMAIL);

      const res = await request(app).get('/v1/auth/verify-email').query({ token: verifyEmailToken }).expect(httpStatus.FOUND);

      const expectedRedirectUrl = new URL(config.frontend.profilePath, config.frontend.url);
      const actualRedirectUrl = new URL(res.headers.location);
      const hashParams = new URLSearchParams(actualRedirectUrl.hash.slice(1));
      const refreshToken = hashParams.get('refreshToken');

      expect(actualRedirectUrl.origin).toBe(expectedRedirectUrl.origin);
      expect(actualRedirectUrl.pathname).toBe(expectedRedirectUrl.pathname);
      expect(hashParams.get('accessToken')).toEqual(expect.any(String));
      expect(refreshToken).toEqual(expect.any(String));
      expect(hashParams.get('verified')).toBe('true');
      expect(hashParams.get('accessExpires')).toEqual(expect.any(String));
      expect(hashParams.get('refreshExpires')).toEqual(expect.any(String));
